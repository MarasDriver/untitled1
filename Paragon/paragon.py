...
def _remote_control_on(rc, demoMode = 0):
    _send("paragon.on(%s,%s)" %(rc, demoMode))
...
def getInstrumentType():
	...
	c,type,sn,other = idnString.split(',')
	#print(idnString)
	return type
...
#  @param demoMode    optional  (PX-5008) Add the ability to open the Paragon-X GUI remotely in demo mode
#  @throw Exception
...
def connect(instAddress, guiAddress = "localhost", guiport = 9000, instPort = 9990, demoMode = 0, rc = 1):
	...
	try:
		_remote_control_on(rc, demoMode)
	
		connStatus = _is_connected("")
		#print ("connect: connStatus = %d" % connStatus)
		if (connStatus < 2 and instAddress == "localhost"):
			error = "ERROR: attempting to connect to localhost when not in demo mode"
			print(error)
		elif connStatus < 2:
...
def paragonset(arg_str, value = None):
	...
	#if {$__instrumentType == "Paragon-X"} {
	if (value != None):
		isBypass = _rstTranslation(arg_str + " " + value)
	else:
